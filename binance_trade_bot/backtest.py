...
class MockBinanceManager(BinanceAPIManager):
    ...
    def buy_alt(self, origin_coin: Coin, target_coin: Coin, buy_price: float):
        ...
        if order_quantity < minimum_order:
            self.logger.info("Unprofitable trade ({order_quantity}) ... Increasing order to ({minimum_order})}")
    ...
    def sell_alt(self, origin_coin: Coin, target_coin: Coin, sell_price: float):
        #print(f"SELL_ALT: _sell_alt({origin_coin}, {target_coin}, {sell_price})")
        origin_symbol = origin_coin.symbol
        target_symbol = target_coin.symbol
...
class MockDatabase(Database):
    def __init__(self, logger: Logger, config: Config):
        super().__init__(logger, config, "sqlite:///data/backtest_crypto_trading.db")
