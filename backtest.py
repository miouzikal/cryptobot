...
if __name__ == "__main__":
    history = []
    start_time = datetime(2021, 7, 1, 0, 0)
    end_time = datetime(2021, 7, 14, 0, 0)
    start_balances = {"BTC": 1000}
    print(f"BACKTEST from {start_time} to {end_time}")
    current_date = start_time.strftime("%d/%m/%Y")
    for manager in backtest(start_time, end_time, 1, 100, start_balances):
